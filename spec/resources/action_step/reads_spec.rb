require 'rails_helper'

RSpec.describe ActionStepResource, type: :resource do
  describe 'serialization' do
    let!(:action_step) { create(:action_step) }

    it 'works' do
      render
      data = jsonapi_data[0]
      expect(data.id).to eq(action_step.id)
      expect(data.jsonapi_type).to eq('action_steps')
    end
  end

  describe 'filtering' do
    let!(:action_step1) { create(:action_step) }
    let!(:action_step2) { create(:action_step) }

    context 'by id' do
      before do
        params[:filter] = { id: { eq: action_step2.id } }
      end

      it 'works' do
        render
        expect(d.map(&:id)).to eq([action_step2.id])
      end
    end
  end

  describe 'sorting' do
    describe 'by id' do
      let!(:action_step1) { create(:action_step) }
      let!(:action_step2) { create(:action_step) }

      context 'when ascending' do
        before do
          params[:sort] = 'id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            action_step1.id,
            action_step2.id
          ])
        end
      end

      context 'when descending' do
        before do
          params[:sort] = '-id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            action_step2.id,
            action_step1.id
          ])
        end
      end
    end
  end

  describe 'sideloading' do
    # ... your tests ...
  end
end
